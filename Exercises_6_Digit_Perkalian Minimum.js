...
yang menerima satu parameter angka. Function akan 
mengembalikan jumlah digit minimal dari angka yang merupakan 
...
function digitPerkalianMinimum(angka) {
  ...
  for (var i = 0 ; i < angka ; i++){
    if ((angka%i) === 0 ){
      simpan = angka/i+ "" +i
      //console.log(angka/i + " x " + i + " = "+simpan)
        if (simpan.length < min){
          min = simpan.length
        }
    }else if (angka === 1){
      min = 2
